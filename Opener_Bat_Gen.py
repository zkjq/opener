...
class SoftwareLauncherApp:
    def __init__(self, master):
        ...
        # 生成 .bat 文件按钮
        self.generate_bat_button = tk.Button(master, text="生成 .bat 文件", command=self.generate_bat_file)
        self.generate_bat_button.pack()
    ...
    def generate_bat_file(self):
        ...
        if self.listbox.size() > 0:
            ...
            for i in range(self.listbox.size()):
                software_info = self.listbox.get(i)
                bat_content += f'start "" {software_info}\n'
            bat_content += "exit\n"

            # 保存 .bat 文件
            with open("software_launcher.bat", "w") as bat_file:
                bat_file.write(bat_content)

            self.update_status(".bat 文件已生成")
        else:
            self.update_status("警告: 请先添加软件路径")
